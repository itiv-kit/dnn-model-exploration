...
def retrain_best_individuals(workload,
    ...
    reeval_datasets = build_dataloader_generators(
    ...
    results = pd.DataFrame(columns=[
        'generation', 'individual', 'accuracy', 'full_acc_after',
        'full_acc_before', 'retrain_epoch_accs', 'weighted_bits',
        'mutation_eta', 'mutation_prob', 'crossover_eta', 'crossover_prob',
        'selection_press', 'cost', 'bits'
    ])

    logger.info("Selecting {} individual(s) for retraining.".format(
    ...
    for index, (cost, individual) in enumerate(individuals_with_cost):
        ...
        full_accuracy_before = accuracy_function(qmodel.model,
        ...
        logger.info(
            "Starting new model (ID #{}) starting acc {:.3f}, starting bits: {}"
            .format(index, full_accuracy_before.float(),
                    individual.weighted_bits))

        epoch_accs = qmodel.retrain(
            train_dataloader_generator=datasets['train'],
        ...
        full_accuracy_after = accuracy_function(qmodel.model,
                                                reevaluate_dataloader,
                                                progress=progress,
                                                title="Reevaluating")
...
if __name__ == "__main__":
    ...
    if os.path.isfile(workload_file):
        ...
        results = retrain_best_individuals(workload, opt.calibration_file,
