...
class SparseConv(CustomConvModule):
    ...
    def __init__(self, old_module, node_name, threshold, block_size, collect_details=True, visualise=False):
        ...
        self.visualization_folder = os.path.join('blocks',
                '{}_{}_{}'.format(self.node_name, self.threshold, self.block_height))
    ...
    def _tensor_to_blocks(self, tensor):
        ...
        return tensor.unfold(0, self.block_height, self.block_height).unfold(1, self.block_width, self.block_width)
    ...
    def _apply_sparsity(self, blocks):
        ...
        if torch.cuda.is_available():
            blocks[mean_blocks < self.threshold, ...] = torch.cuda.FloatTensor(
                self.block_height, self.block_width).fill_(0)
        else:
        ...
        add_forward_data(self.node_name, min_of_blocks.item(), max_of_blocks.item(),
    ...
    Otherwise the block will remain unchanged.
        ...
        inp_unf = torch.nn.functional.pad(
            inp_unf, (0, padding_right, 0, padding_bottom), "constant", torch.nan)
        ...
        for batch in range(inp_unf.size(0)):
            ...
            inp_unf[batch] = self._blocks_to_tensor(sparse_blocks, padded_width, padded_height)

            if self.visualize:
                filename = os.path.join(self.visualization_folder,
                                        '{:05d}.png'.format(self.sample_counter))
                tensor2d_to_heatmap_comparison(im2col_2dtensor.to('cpu'), inp_unf[batch].to('cpu'), filename, "")
