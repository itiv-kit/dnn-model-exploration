...
def reevaluate_individuals(workload: Workload, calibration_file: str, results_path: str,
    ...
    results = pd.DataFrame(columns=['generation', 'individual', 'accuracy', 'acc_full',
    ...
    logger.info("Selecting {} individual(s) for reevaluation with the full dataset.".format(len(individuals_with_cost)))
    ...
    for i, (cost, individual) in enumerate(individuals_with_cost):
        ...
        full_accuracy = accuracy_function(qmodel.model, reevaluate_dataloader, progress=progress,
                                          title="Reevaluating {}/{}".format(i+1, len(individuals_with_cost)))

        logger.info("Done with ind {} / {}, accuracy is {:.4f}, was before {:.4f} at cost {:.4f} ({} bit)".format(
                i+1, len(individuals_with_cost), full_accuracy, individual.accuracy, cost, individual.weighted_bits))
...
if __name__ == "__main__":
    ...
    parser.add_argument("results_path", help="Path to the results file or folder to be evaluated")
    ...
    if os.path.isfile(workload_file):
        ...
        results = reevaluate_individuals(workload, opt.calibration_file, opt.results_path, 
                                         opt.top_elements, opt.progress, opt.verbose)
